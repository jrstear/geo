...
- Spot instances acceptable for experiments; on-demand preferred for production. Spot
  interruption loses only the current stage — outputs are synced to S3 after each stage
  and the pipeline resumes automatically on reboot (see `infra/ec2/scripts/odm-run.sh`).
...
s3://geo-odm-{account_id}/
  projects/{project_id}/
    inputs/
      ...
      gcp_list.txt      # GCP-only, ODM metric CRS (e.g. EPSG:6528 NM Central; transform.yaml `odm_crs`)
      chk_list.txt      # CHK-only, same CRS as gcp_list.txt (used by rmse_calc.py)
    runs/{run_id}/
...
|---|---|
| `geo/odm` | ODM image (based on `opendronemap/odm:3.3.0`), with `run.sh` adapter script added |
...
# sight.py provides shared parsing utilities used by rmse_calc_batch.py
COPY sight.py .
  ...
  terraform/
    ...
    environments/
      dev/
        ...
        terraform.tfvars # environment = "dev", region = "us-west-2"
...
- **rmse_calc_batch.py**: Not yet written. It must read `reconstruction.json` +
  `chk_list.txt` (CHK-only, ODM metric CRS — e.g. EPSG:6528 for NM Central) from S3 and write `rmse_report.json`.

...
s3://geo-odm-{account_id}/
  projects/{project_id}/
    inputs/
      ...
      {job}_control.txt # GCP-only, EPSG:32613 metres (produced by convert_survey.py)
      {job}_check.txt   # CHK-only, EPSG:32613 metres (used by rmse_calc.py)
    runs/{run_id}/
...
- **rmse_calc_batch.py**: Not yet written. It must read `reconstruction.json` +
  `{job}_check.txt` (CHK-only, EPSG:32613 metres) from S3 and write `rmse_report.json`.

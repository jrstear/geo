...
`sight.py` reads an Emlid rover CSV plus drone images and produces `gcp_list.txt`
...
Reuse `parse_survey_csv()` from `sight.py`. Import it directly:

```python
import sys, os
sys.path.insert(0, os.path.dirname(__file__))   # TargetSighter/
from sight import parse_survey_csv, _cs_name_to_epsg
```
...
1. **Define experiment matrix** — parametrise over:
   ...
   - `nadir_weight`: float (controls pipeline image ordering; re-run `sight.py`
...
running `sight.py` with `--nadir-weight {nw}` before invoking the driver.
...
| File | Path |
|---|---|
| Existing pipeline (reference) | `/Users/jrstear/git/geo/TargetSighter/sight.py` |
| `parse_survey_csv()` | In `sight.py` — import directly |
| `_cs_name_to_epsg()` | In `sight.py` — import directly |
| `project_pixel_mode_b()` | In `sight.py` — reference for reconstruction.json camera math |
...
1. Read this spec and the reference file `sight.py`.
